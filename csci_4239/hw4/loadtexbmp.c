 ...
 *  Load texture from BMP file
 */
unsigned int LoadHeightmapBMP(const char* file, float **cpu_buffer, unsigned int slot)
{
   unsigned int   texture;    // Texture name
   FILE*          f;          // File pointer
   unsigned short magic;      // Image magic
   ...
   size = 3*dx*dy;
   image = (unsigned char*) malloc(size);
   *cpu_buffer = (float*) malloc(sizeof(float)*size);
   if (!image) Fatal("Cannot allocate %d bytes of memory for image %s\n",size,file);
   //  Seek to and read image
   if (fseek(f,off,SEEK_SET) || fread(image,size,1,f)!=1) Fatal("Error reading data from image %s\n",file);
   fclose(f);
   //  Reverse colors (BGR -> RGB)
   //  Also, add colors to cpu_side buffer
   for (k=0;k<size;k+=3)
   {
      unsigned char temp = image[k];
      image[k]   = image[k+2];
      image[k+2] = temp;
      (*cpu_buffer)[k / 3] = (float)temp/255.0;
   }
