...
//ctag
int do_crypt(char* path, char* buffer, int action, char* key_str){
    /* Buffers */
	...
	if (action == READ) {
		...
		for(;;){
			/* Read Block */
			inlen = fread(inbuf, sizeof(*inbuf), BLOCKSIZE, file);
			if(inlen <= 0){
				/* EOF -> Break Loop */
				break;
			}
			...
			/* Write Block */
			strncpy(buffer + offset, (char*)outbuf, sizeof(*outbuf));
			offset += sizeof(*outbuf);
		}
		    ...
		    struct fuse_file_info *fi)
{
    ...
    /* Buffers */
    unsigned char encrypted_buffer[BLOCKSIZE];
    int encrypted_length;
    /* Allow enough space in output buffer for additional cipher block */
    unsigned char decrypted_buffer[BLOCKSIZE + EVP_MAX_BLOCK_LENGTH];
	...
	/* Build Key from String */
	i = EVP_BytesToKey(EVP_aes_256_cbc(), EVP_sha1(), NULL,
			   (unsigned char*)private_data->keyphrase, strlen(private_data->keyphrase), nrounds, key, iv);
	if (i != 32) {
		/* Error */
		fprintf(stderr, "Key size is %d bits - should be 256 bits\n", i*8);
		return 0;
	...
	/* Init Engine */
	EVP_CIPHER_CTX_init(&ctx);
	EVP_CipherInit_ex(&ctx, EVP_aes_256_cbc(), NULL, key, iv, READ);

	/* Loop through Input File*/
	for(;;){
		/* Read Block */
		encrypted_length = fread(encrypted_buffer, sizeof(*encrypted_buffer), BLOCKSIZE, file);
		...
		/* If in cipher mode, perform cipher transform on block */
		if(!EVP_CipherUpdate(&ctx, decrypted_buffer, &decrypted_length, encrypted_buffer, encrypted_length))
		{
			/* Error */
			EVP_CIPHER_CTX_cleanup(&ctx);
			return 0;
		...
		/* Write Block */
		strncpy(buf + output_length, (char*)decrypted_buffer, decrypted_length);
		output_length += decrypted_length;
	}

	/* Handle remaining cipher block + padding */
	if(!EVP_CipherFinal_ex(&ctx, decrypted_buffer, &decrypted_length)){
		/* Error */
		EVP_CIPHER_CTX_cleanup(&ctx);
		return 0;
	}
